// v7 add NFT contract integration
...
interface ILiquiswapNFT {
    function balanceOf(address) external view returns (uint);
    function awardNFT(address) external;
}


// note: just refers to native so MATIC on polygon
...
contract LiquiswapV7 is AutomationCompatibleInterface {
    ...
    event EarnedNFT(address indexed user);
    event Donation(address indexed charity, uint amount);


    // the Liquiswap NFT contract
    address liquiswapNFT;
    ...
    IWETHGateway constant WETHGateway = IWETHGateway(0x2a58E9bbb5434FdA7FF78051a4B82cb0EF669C17);
    
    // Aave aToken contract - WMATIC-AToken-Polygon
    IERC20 constant AaveWMatic = IERC20(0x89a6AE840b3F8f489418933A220315eeA36d11fF);    
...
// /testing
    ...
    /// @dev set the address of the Liquiswap NFT contract
    function setLiquiswapNFT(address _addr) external onlyOwners {
        liquiswapNFT = _addr;
    }
    ...
    function addUser() public {
        ...
        users[msg.sender] = user({usersIndexPosition: usersIndex.length, principalMATIC: 0, liquidationPrice: -1, liquidationSharesIn: 0, balanceDAI: 0, wasLiquidated: false});
    ...
    /// @dev users can adjust the principal
    ...
    /// @dev mints a user NFT on the Liquiswap NFT contract
    function awardNFT(address _user) internal {
        ILiquiswapNFT(liquiswapNFT).awardNFT(_user);   
    }


    /// @dev returns the total number of NFTs a user has been awarded
    function getNumNFTs(address _user) public view returns (uint) {
        return ILiquiswapNFT(liquiswapNFT).balanceOf(_user);
    }


    function getNumNFTs() external view returns (uint) {
        return getNumNFTs(msg.sender);
    }
    ...
    // award / mint an NFT to address
    function zdevAwardNFT(address _addr) external onlyOwners {
        awardNFT(_addr);
    }
